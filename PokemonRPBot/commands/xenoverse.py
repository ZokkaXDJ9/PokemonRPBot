...
def find_movelist_filename(normalized: str, folder: str = os.path.join("data", "secret/xenoverse")) -> str:
...
class PersistentPokemonAbilitiesButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        fn = find_movelist_filename(norm, "data/secret/xenoverse")
        if not fn:
...
class XenoCog(commands.Cog):
    ...
    @app_commands.command(name="xenoverse", description="Show details for a Pokémon")
    async def pokemon(self, interaction: discord.Interaction, pokemon: str):
        norm = normalize_name(pokemon)
        folder = os.path.join("data", "secret/xenoverse")
