...
def get_field_value(move: dict, keys: list, default):
    """
    Searches the move dictionary for any key matching one of the keys (case-insensitive).
...
class MTemplateCommand(commands.Cog):
    ...
    @app_commands.command(
        name="mtemplate",
        description="Display a standardized move template (version 2 format) for editing."
    )
    @app_commands.autocomplete(move_name=move_name_autocomplete)
    async def mtemplate(self, interaction: discord.Interaction, move_name: str):
        ...
        standardized_move = {
            "name": get_field_value(move, ["name", "Name"], "Template"),
            "type": get_field_value(move, ["type", "Type"], "Typeless/any Type"),
            "power": get_field_value(move, ["power", "Power"], 0),
            "damage": get_field_value(move, ["damage", "damage1", "Damage1"], "Strength/Special etc."),
            "accuracy": get_field_value(move, ["accuracy", "accuracy1", "Accuracy1"], "Dexterity/Insight etc."),
            "target": get_field_value(move, ["target", "Target"], "Foe/User/etc"),
            "effect": get_field_value(move, ["effect", "Effect"], "Effect Description"),
            "description": get_field_value(move, ["description", "Description"], "Some roleplay description"),
            "category": get_field_value(move, ["category", "Category"], "Physical/Special/Support")
        }

        # Format the standardized move as indented JSON.
        formatted_json = json.dumps(standardized_move, indent=4)
        # Send the JSON inside a code block with JSON syntax highlighting.
